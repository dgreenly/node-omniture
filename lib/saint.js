...
p.importCreateJob = function(jobDescription, callback){
	...
	this.request(this.methods.importCreateJob, jobDescription, function(err, data){
		console.log("Here is DATA: "+data+": END DATA");
		console.log("Here is ERR: "+err+": END ERR");
		try {
			var json = JSON.parse(data);
		}catch (exception) {
			var json = {
				errors: [
					'importCreateJob: Unable to decode json. This may indicate a problem connecting to Adobe. There may be more information in the server log.'
				]
			}
			console.log('Error decoding json string: Line 137 ' + data);
		}
...
p.importCommitJob = function(jobId, callback){
	...
	this.request(this.methods.importCommitJob, {job_id: jobId}, function(err, data){
		try {
			...
			var json = {
				errors: [
					'Unable to decode json. This may indicate a problem connecting to Adobe. There may be more information in the server log.'
				]
			}
			console.log('Error decoding json string: ' + data);
		}
		if(err || data.toLowerCase() == 'failed' || typeof json.errors != 'undefined'){ 
			var message = '';
			if(err && err.message){
