...
p.importCreateJob = function(jobDescription, callback){
	...
	this.request(this.methods.importCreateJob, jobDescription, function(err, data){
		try{
			...
			var json = {
				errors: [
					'importCreateJob: Unable to decode json. There may be more information in the server log.'
				]
			}
			console.log('importCreateJob: Error decoding json string: ' + data);
		}
...
p.importPopulateJob = function(jobSlices, callback){
	...
	this.request(this.methods.importPopulateJob, jobSlices, function(err, data){
		try{
			...
			var json = {
				errors: [
					'importPopulateJob: Unable to decode json. There may be more information in the server log.'
				]
			}
			console.log('importPopulateJob: Error decoding json string: ' + data);
		}
...
p.importCommitJob = function(jobId, callback){
	...
	this.request(this.methods.importCommitJob, {job_id: jobId}, function(err, data){
		try {
			...
			var json = {
				errors: [
					'importCommitJob: Unable to decode json. There may be more information in the server log.'
				]
			}
			console.log('importCommitJob: Error decoding json string: ' + data);
		}
		if(err || data.toLowerCase() == 'failed' || typeof json.errors != 'undefined'){ 
			var message = '';
			if(err && err.message){
